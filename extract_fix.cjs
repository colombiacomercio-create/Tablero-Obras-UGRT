...
async function run() {
  ...
  filteredData.forEach(d => {
    ...
    let matchedKey = rawLoc ? Object.keys(locStats).find(k => normalizar(k) === rawLoc) : null;
  ...
  fs.writeFileSync("datos_verdaderos_arreglados.json", JSON.stringify({ rankingIDC, alertasResumen }, null, 2));
}
run();
