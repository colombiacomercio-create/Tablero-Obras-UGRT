...
async function run() {
  ...
  // KPI Globales
  let terminadas = 0, suspendidas = 0, enEjec = 0, sinCrono = 0;
  let km=0, m2=0, ml=0, huecos=0;
  filteredData.forEach(d => {
    const e = d.estado ? String(d.estado).toUpperCase().trim() : '';
    if (e === 'TERMINADO') { terminadas++; km+=Number(d.km_carril)||0; m2+=Number(d.m2)||0; ml+=Number(d.ml)||0; huecos+=Number(d.huecos)||0; }
    else if (e === 'SUSPENDIDO') suspendidas++;
    else if (e === 'EN EJECUCIÓN' || e === 'EN EJECUCION' || e === 'POR INICIAR') enEjec++;
    else if (e === 'SIN CRONOGRAMA' || e === 'SIN CRONOGRAMA ') sinCrono++;
  });

  // Ranking IDC
  const locStats = {};
  LOCALIDADES.forEach(loc => locStats[loc] = { loc, univ:0, prog:0, term:0, susp:0, venc:0 });
  const today = new Date('2026-05-29'); // Approx
  ...
  let out = '=== GLOBALES ===\n';
  out += Frentes: \nTerminadas: \nEn Ejec/Por Iniciar: \nSin Cronograma: \nSuspendidas: \n;
  out += Físicas: Km: , M2: , ML: \n\n;

  out += '=== RANKING IDC ===\n';
  rankingIDC.forEach((r, i) => {
    out += ${i+1}.  - Univ:, Prog:, Term:, Susp:, Venc:, Pct:%\n;
  });

  out += '\n=== ALERTAS ===\n';
  alertasResumen.forEach((r, i) => {
    out += ${i+1}.  - Alertas:, SI:, NO:, Pct:%\n;
  });

  fs.writeFileSync('datos_boletin.txt', out);
